...
| `flowstate doctor` | Pure-Python health checks — 6 checks (manifest integrity, memory schema, root resolution, claude CLI, stale status, orphan files) |
...
- **[sqlite-vec](https://github.com/asg017/sqlite-vec)** by Alex Garcia & **[fastembed](https://github.com/qdrant/fastembed)** by Qdrant — A SQLite vector-search extension and an ONNX-based local embedding library. `sqlite-vec` is a **core dependency** (always installed) and stores the `memories_vec` KNN index inside `memory.db`; the embedding provider `fastembed` (`bge-small-en-v1.5`) is the **only** piece behind the optional `[semantic]` extra. With the extra installed, the two together back FlowState's semantic memory retrieval — fully local, no network at query time (v0.6). Without it, retrieval degrades to FTS5/BM25 and `sqlite-vec` remains available for the vector store.
