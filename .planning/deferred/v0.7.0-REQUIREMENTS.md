...
> **DEFERRED 2026-07-10.** This milestone was scoped and roadmapped, then deferred behind
> **v0.6.1 "Make the Names Real"** (fix the adapter stubs first — a harness whose enforcement
> layer cannot fail can't be meaningfully benchmarked). This spec is preserved verbatim as the
> waiting requirements. When v0.6.1 ships, run `/gsd-new-milestone` for v0.7.0; the roadmapper
> continues numbering from v0.6.1's last phase, so the phase references below (12–17) become
> **16–21** (v0.6.1 grew to 4 phases when GSD full-runtime bundling was added). Update the
> Traceability table accordingly at that time. Active requirements live in
> `.planning/REQUIREMENTS.md` (v0.6.1).
