...
- ✓ **SBX-03..04** *(v0.9.0 / Phase 24)*: the sandbox seam now fires on real runs — 6 agent-directed subprocess sites route through `wrap()`: `bridge.py:309` + `distiller.py:96` at surface `"llm"` (tier threaded from `ProjectPreferences.sandbox` via `_make_bridge`, mirroring `enable_prompt_caching_1h`), `tools/base.py:78` + `pack.py:119` at `"tool"` (threaded from the in-scope `state.preferences.sandbox`), `gsd_vendor.py:330/386` at `"tool"` DEFAULT observe (not project-scoped — the GSD-refresh path has no root). `discipline.py:52/62/72/103` git/pytest reads deliberately LEFT BARE with a sourced SBX-03/D-01 comment at both physical sites. New defaulted `ProjectPreferences.sandbox: str = "observe"` (+ `BridgeConfig.sandbox`) — backward-compat load, NO `_migrate_state` change. **Env-scrub goes LIVE by default** (`observe`); bridge env-prep ordering preserved (CLAUDECODE popped, 1h-cache var set, both survive the scrub); auth-survival tested at both claude sites via `_AUTH_EXEMPT`. Code review: 0 Critical, WR-1 fixed (lazy `.bridge` now threads the tier — no silent confine→observe downgrade); WR-2 recorded for Phase 25 (npm at observe strips `*_TOKEN` → would break a private-registry `NPM_TOKEN`; widening the exemption is a posture decision). 1316 tests @ 91.37%. NO confine-tier hardening / WR-03 confirmations (Phase 25). — v0.9.0 / Phase 24
- ✓ **SBX-01..02** *(v0.9.0 / Phase 23)*: native `flowstate/sandbox.py` guardrail seam — `wrap(cmd, surface, project_root, env, *, tier="observe") -> (argv, env)` never spawns/execs the target (D-04); non-blocking `observe` tier via a case-insensitive secret-denylist `_scrub_env` WITH an `_AUTH_EXEMPT` carve-out (never strips `ANTHROPIC_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN`/`CLAUDE_CONFIG_DIR` — D-01, named regression test); golden-tested `build_macos_profile` (allow-default+selective-deny SBPL, path-escaped against injection) + `build_linux_bwrap_args`; Landlock ctypes helper (syscalls 444/445/446, return-codes checked → fail-closed on restriction failure) + functional `check_bwrap_available` smoke test + D-03 two-rung ladder (bwrap+landlock → bwrap-only → observe). **SBX-01 Linux spike PARITY PROVEN** on a real kernel (6.12, Landlock ABI v6): fs confinement (EACCES+EROFS) AND confined `claude --print` auth both demonstrated (`23-SPIKE-LINUX.md`); two production-shape confirmations (writable HOME, file-path credential) flagged for Phase 25. NO call site wired + NO `ProjectPreferences.sandbox` field (Phase 24 scope). No new runtime dep (stdlib ctypes/subprocess). 1302 tests @ 91.33%, sandbox.py 95%.
...
---
*Last updated: 2026-07-12 after Phase 24 (Thread the Seam + Config, v0.9.0) — SBX-03..04 shipped + verified (8/8) and moved to Validated: 6 agent-directed subprocess sites wrapped, defaulted `ProjectPreferences.sandbox` field (no migration), env-scrub live by default, auth preserved at both claude sites; code review 0 Critical (WR-1 lazy-bridge tier fixed; WR-2 npm-NPM_TOKEN recorded for Phase 25); 1316 tests @ 91.37%. v0.9.0 now 2/3 phases. Next: Phase 25 (confine-tier production profiles + E2E denial proof + the two WR-03 confirmations). v0.8.0 Phase-22 verdict debt still parked/owed.*
